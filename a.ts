...
`
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('extension.toggleRegexPreview', toggleRegexPreview)
  )
  ...
  context.subscriptions.push(
    vscode.window.onDidChangeActiveTextEditor(() => updateDecorators(findRegexEditor()))
  )
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('regexpreview.toggleGM', () => {
  ...
  function updateToggleGM() {
    ...
    toggleGM.tooltip = addGMEnabled
      ? 'Click to stop adding global and multiline (/gm) options to regexes for evaluation with example text.'
      : 'Click to add global and multiline (/gm) options to regexes for evaluation with example text.'
  }
  updateToggleGM()
  ...
  function openLoremIpsum(column: number, initialRegexMatch?: RegexMatch) {
    return vscode.workspace
      .openTextDocument({ language: 'text', content: matchesFileContent })
      .then(document => {
  ...
  function updateDecorators(regexEditor?: vscode.TextEditor, initialRegexMatch?: RegexMatch) {
    ...
    if (
      regexEditor &&
      initialRegexMatch &&
      initialRegexMatch.document &&
      initialRegexMatch.document.uri.toString() === regexEditor.document.uri.toString()
    ) {
  ...
  function applyDecorator(
    matchEditor: vscode.TextEditor,
    initialRegexEditor?: vscode.TextEditor,
    initialRegexMatch?: RegexMatch
  ) {
  ...
  class RegexMatchDecorator {
    ...
    constructor(private matchEditor: vscode.TextEditor) {
      ...
      this.disposables.push(
        vscode.workspace.onDidChangeTextDocument(e => {
          if (
            (this.stableRegexEditor && e.document === this.stableRegexEditor.document) ||
            e.document === matchEditor.document
          ) {
            this.update()
          }
        })
    ...
    public update() {
      ...
      const matches =
        regex && regexEditor !== this.matchEditor
          ? findMatches(regex, this.matchEditor.document)
          : []
      this.matchEditor.setDecorations(
        matchHighlight,
        matches.map(match => match.range)
      )

      if (regexEditor) {
        regexEditor.setDecorations(
          regexHighlight,
          (this.stableRegexMatch || regexEditor !== vscode.window.activeTextEditor) && regex
            ? [regex.range]
            : []
        )
      }
  ...
  function findRegexAtCaret(editor: vscode.TextEditor): RegexMatch | undefined {
    ...
    while (
      ...
      match.index + match[1].length + match[2].length < anchor.character
    );
